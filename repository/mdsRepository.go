...
func (r *mdsRepository) Create(entry *model.MdsEntry) (int, error) {
	...
	result, err := r.db.Exec("INSERT INTO mdsListing (name, comments, effectiveFrom, effectiveTo, isPPAgreed, documentPath, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?)",
...
func (r *mdsRepository) GetAll() ([]model.MdsEntry, error) {
	rows, err := r.db.Query("SELECT id, name, comments, effectiveFrom, effectiveTo, isPPAgreed, documentPath, createdAt FROM mdsListing")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		err := rows.Scan(&entry.ID, &entry.Name, &entry.Comments, &entry.EffectiveFrom, &entry.EffectiveTo, &entry.IsPPAgreed, &entry.DocumentPath, &entry.CreatedAt)
		if err != nil {
			return nil, err
		}
